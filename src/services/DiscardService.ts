...
import { TileMovementService } from './TileMovementService';
import { TileSupplyService, TileSupplyState } from './TileSupplyService';
...
/**
 * Centralized service for managing discard operations.
 * Handles removing tiles from rack or board and coordinating with tile supply.
 */
export class DiscardService {
    ...
    static removeForDiscard(
        ...
        const rackIndex = TileMovementService.findTileInRack(rack, tileId);
        if (rackIndex !== null) {
        ...
        const boardPos = TileMovementService.findTilePosition(board, tileId);
        if (boardPos) {
     ...
     * @param board - Current board state
     * @param placementHistory - Current placement history
     * @param supplyState - Current bag/rack/discard state (rack here may differ from param rack if updated)
    ...
    static discardAndDraw(
        ...
        const removeResult = this.removeForDiscard(rack, board, placementHistory, tileId);
        if (!removeResult) {
            return null;
        }

        // Determine source rack index for TileSupplyService.discardAndDraw
        const sourceRackIndex = TileMovementService.findTileInRack(rack, tileId);
        ...
        const drawResult = TileSupplyService.discardAndDraw(
